...
def calculate_social_distancing(vid_path, net, output_dir, output_vid, ln1, detection_rate):
    ...
    bird_movie = cv2.VideoWriter("./output_vid/bird_eye_view.avi", fourcc, fps_, (int(width * scale_w), int(height * scale_h)))
    ...
    ####################################################################################
        ...
        dis_to_bench_mat, bench_pts_mat, complete_bench_mat = utills.get_bench_distances(boxes1, person_points, bpts, bench_points, ids, distance_w, distance_h)
        ...
        bird_image = plot.bird_eye_view(frame, distances_mat, person_points, dis_to_bench_mat, bench_points, ids, scale_w, scale_h, risk_count)
        # img = plot.social_distancing_view(frame1, bxs_mat, boxes1, bench_bxs_mat, benches1, speeds, risk_count)
        img = plot.social_distancing_view(frame1, bxs_mat, boxes1, bench_pts_mat, bpts, ids, speeds, risk_count, count, frame_time)
...
if __name__== "__main__":
    ...
    parser.add_argument('-v', '--video_path', action='store', dest='video_path', default='./data/example.avi' ,
